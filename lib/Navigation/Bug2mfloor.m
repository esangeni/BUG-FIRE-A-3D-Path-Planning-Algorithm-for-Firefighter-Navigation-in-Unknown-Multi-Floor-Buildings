%BUG2 Bug navigation class - multifloor
%
%--------------------------------------------------------------------------
...
%
%--------------------------------------------------------------------------
% MODIFIED BY EUDALD SANGENIS RAFART FOR MAE, UCI, DEPARTMENT BUILDING
% SCENARIO MULTIFLOOR PLAN
 

classdef Bug2mfloor < Navigation

    properties(Access=protected)
        H       % hit points
        j       % number of hit points
        mline   % line from starting position to goal
        mlineRobot   % line from starting position to goal
        step    % state, in step 1 or step 2 of algorithm
        edge    % edge list
        k       % edge index

        stopCells % New property to define stop cells
    end

    methods

        function bug = Bug2mfloor(varargin)
            ...
            bug.j = 1;
            bug.step = 1;
            bug.stopCells = []; % Initialize stop cells
        end

        function setStopCells(bug, cells)
            % Set stop cells
            % cells: Nx2 matrix, where each row is a [x, y] position
            bug.stopCells = cells;
        end

        function [pp, stopReached, goalReached] = query(bug, startInit, startRobot, goal, varargin)
            ...
            bug.start = startRobot; bug.goal = [];
%             bug.start = []; bug.goal = [];
%             bug.checkquery(start, goal);
            bug.checkquery(startRobot, goal);
...
%                 bug.goal(1), bug.goal(2));
            bug.mline = homline(startInit(1), startInit(2), ...
                bug.goal(1), bug.goal(2));
            bug.mline = bug.mline / norm(bug.mline(1:2));

            bug.mlineRobot = homline(startRobot(1), startRobot(2), ...
                bug.goal(1), bug.goal(2));
            bug.mlineRobot = bug.mlineRobot / norm(bug.mlineRobot(1:2));
            
            if opt.animate
                fig = figure;
                screenSize = get(0, 'ScreenSize');
                set(fig, 'Position', 0.925*screenSize);

                bug.plot();
                bug.plot_mline();
                bug.plotStopCells();  % Plot the stop cells in blue
            end
...
%             path = startRobot(:);
            while true
                ...
                [robot, goalReached] = bug.next(robot); % Here robot changes from [54,111] to [36,110] - WHY?

                % are we there yet?
                if isempty(robot)
                    stopReached = true;  % Robot stopped due to reaching a stop cell
                    disp("Emergency Stair Found.")
                    % yes, exit the loop
                    break
                else
...
%                     plot([start(1) start(1)], [ymin ymax], 'k--');
                    plot([startInit(1) startInit(1)], [ymin ymax], 'k--');
                else
                    x = [xmin xmax]';
                    y = -[x [1;1]] * [bug.mline(1); bug.mline(3)] / bug.mline(2);
                    plot(x, y, ls);
                    axis equal
                    xlim([0,80]); ylim([0,340]);
                end
        end

        function plotStopCells(bug)
            % Plot the stop cells in blue
            if ~isempty(bug.stopCells)
                hold on;
                plot(bug.stopCells(:, 1), bug.stopCells(:, 2), 'b-', 'LineWidth', 5, 'MarkerFaceColor', 'blue');
            end
        end
        ...
        function [n, goalReached] = next(bug, robot)
            ...
            if ismember(robot', bug.stopCells, 'rows')
                bug.message('Stopped at predefined stop cell: (%d,%d)', robot);
